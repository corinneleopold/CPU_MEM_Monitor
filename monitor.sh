...
function check_arguments {
	...
	#Extract the memory threshold
	#The number of arguments should be greater than 7
	if [ "$#" -ge 7 ]; then
		MEM_THRESHOLD=$6
	fi
}
...
function generate_report {
	...
	if [[ "$report_count" -eq "$MAXIMUM_REPORTS" ]]; then 	
		$(cd ./reports_dir && ls -t | tail -1 | xargs rm)
	fi
...
#	VmRSS_String=$(echo $(grep -i 'VmRSS:' /proc/$PID/status))

	mem_usage=$(grep -i 'VmRSS' /proc/$PID/status | awk '{ print $2 }')
...
function notify
{
	...
	if [[ "$cpu_usage_int" -gt "$CPU_THESHOLD" ]]; then
		#Send email
		/usr/bin/mailx -s "Usage exceeded" $USER < $report
